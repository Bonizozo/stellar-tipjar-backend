...
use crate::helpers::test_data::{generate_creators, generate_tips, CreatorBuilder, TipBuilder};
use crate::helpers::{ConcurrentTestRunner, PerformanceMetrics, TestContext};
...
async fn test_complete_tip_flow() {
    ...
    // Create a creator
    let creator = ctx
        .create_creator("tip_flow_creator", "GFLOW123", "flow@test.com")
        .await;
    assert_eq!(creator["username"], "tip_flow_creator");

    // Record a successful tip
    let response = ctx
        .record_tip_with_mock("tip_flow_creator", "25.5", "TXFLOW123", true)
        .await;
...
async fn test_multiple_tips_same_creator() {
    ...
    // Create creator
    ctx.create_creator("multi_tip_creator", "GMULTI123", "multi@test.com")
        .await;
    ...
    for (i, amount) in tip_amounts.iter().enumerate() {
        ...
        let response = ctx
            .record_tip_with_mock("multi_tip_creator", amount, &tx_hash, true)
            .await;
        response.assert_status(StatusCode::CREATED);
    }
    ...
    let recorded_amounts: Vec<String> = tips
        .iter()
...
async fn test_tip_nonexistent_transaction() {
    ...
    // Create creator
    ctx.create_creator("notx_creator", "GNOTX123", "notx@test.com")
        .await;

    // Mock non-existent transaction
    ctx.stellar_mocks
        .mock_nonexistent_transaction("TXNOTEXIST123");
...
async fn test_tip_stellar_network_timeout() {
    ...
    // Create creator
    ctx.create_creator("timeout_creator", "GTIMEOUT123", "timeout@test.com")
        .await;
...
async fn test_tip_performance() {
    ...
    // Measure tip recording performance. Timed manually rather than via
    // `ctx.measure_time(...)`: that takes `&self`, but `record_tip_with_mock`
    // needs `&mut self`, and a closure can't hold both borrows of `ctx` at once.
    let start = std::time::Instant::now();
    let response = ctx
        .record_tip_with_mock("perf_creator", "10.0", "TXPERF123", true)
        .await;
...
async fn test_bulk_tip_processing() {
    ...
    for (i, creator) in creators.iter().enumerate() {
        ...
        for j in 0..3 {
            ...
            let response = ctx
                .record_tip_with_mock(username, &amount, &tx_hash, true)
                .await;
            response.assert_status(StatusCode::CREATED);
        }
    }
...
async fn test_tip_data_integrity() {
    ...
    // Create creator
    let creator = ctx
        .create_creator("integrity_creator", "GINT123", "integrity@test.com")
        .await;
    let creator_id = creator["id"].as_str().unwrap();

    // Record tip
    let response = ctx
        .record_tip_with_mock("integrity_creator", "42.75", "TXINT123", true)
        .await;
    response.assert_status(StatusCode::CREATED);
...
async fn test_tip_ordering() {
    ...
    // Create creator
    ctx.create_creator("order_creator", "GORDER123", "order@test.com")
        .await;
    ...
    for (tx_hash, amount) in &tip_data {
        let response = ctx
            .record_tip_with_mock("order_creator", amount, tx_hash, true)
            .await;
        response.assert_status(StatusCode::CREATED);
    ...
    for i in 0..tips.len() - 1 {
        ...
        assert!(
            ...
            "Tips should be ordered by creation time (newest first)"
        );
    }
