...
async fn test_tip_recording_performance() {
    ...
    ctx.create_creator("tip_perf_creator", "GTIPPERF123", "tipperf@test.com")
        .await;

    // Measure tip recording performance
    let start = Instant::now();
    let response = ctx
        .record_tip_with_mock("tip_perf_creator", "10.0", "TXTIPPERF123", true)
        .await;
    let duration = start.elapsed();
...
async fn test_bulk_creator_creation_performance() {
    ...
    println!(
        "Created {} creators in {:?} (avg: {:?} per creator)",
    ...
    assert!(
        ...
        "Average creator creation time {:?} too slow",
        avg_duration
    );
...
async fn test_bulk_tip_recording_performance() {
    ...
    // Create creator
    ctx.create_creator("bulk_tip_creator", "GBULKTIP123", "bulktip@test.com")
        .await;
    ...
    assert!(
        ...
        "Average tip recording time {:?} too slow",
        avg_duration
    );
...
async fn test_concurrent_operations_performance() {
    ...
    println!(
        "Completed {} concurrent operations in {:?} (avg: {:?} per operation)",
    ...
    assert!(
        ...
        "Average concurrent operation time {:?} too slow",
        avg_duration
    );
...
async fn test_database_query_efficiency() {
    ...
    ctx.create_creator("query_test_creator", "GQUERY123", "query@test.com")
        .await;
...
async fn test_tip_list_retrieval_performance() {
    ...
    ctx.create_creator("tip_list_creator", "GTIPLIST123", "tiplist@test.com")
        .await;
...
async fn test_memory_usage_under_load() {
    ...
    println!(
        "Completed {} operations under load in {:?}",
    ...
    assert!(
        avg_duration < Duration::from_millis(200),
        "Average operation time under load {:?} too slow",
        avg_duration
    );
...
async fn test_response_size_optimization() {
    ...
    ctx.create_creator("response_size_creator", "GRESPSIZE123", "respsize@test.com")
        .await;
    ...
    assert!(
        ...
        "Large response retrieval took {:?}, too slow",
        duration
    );
...
async fn test_stellar_api_timeout_handling() {
    ...
    // Create creator
    ctx.create_creator(
        "timeout_perf_creator",
        "GTIMEOUTPERF123",
        "timeoutperf@test.com",
    )
    .await;
