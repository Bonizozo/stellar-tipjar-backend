...
async fn create(
    ...
    let tip = ctrl::create(&state.db, body)
        .await
...
async fn update(
    ...
    let tip = ctrl::update(&state.db, id, body)
        .await
        .map_err(|e| match e {
            sqlx::Error::RowNotFound => {
                AppError::unauthorized("Scheduled tip not found or not pending")
            }
            other => AppError::from(other),
        })?;
    Ok((StatusCode::OK, Json(ScheduledTipResponse::from(tip))))
...
async fn cancel(
    ...
    if cancelled {
        ...
        Err(AppError::unauthorized(
            "Scheduled tip not found or not pending",
        ))
    }
}
