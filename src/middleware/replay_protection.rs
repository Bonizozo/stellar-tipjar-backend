use crate::security::replay_protection::{ReplayProtectionError, ReplayProtectionService};
...
impl ReplayHeaders {
    ...
    pub fn parse_timestamp(&self) -> Result<DateTime<Utc>, ReplayProtectionError> {
        let timestamp_str = self
            .x_timestamp
            .as_ref()
            .ok_or(ReplayProtectionError::MissingNonceOrTimestamp)?;
        ...
        if let Ok(millis) = timestamp_str.parse::<i64>() {
            return Ok(Utc
                .timestamp_opt(millis / 1000, (millis % 1000 * 1_000_000) as u32)
                .single()
                .unwrap_or_else(|| Utc::now()));
        }

        // Try parsing as ISO 8601
        timestamp_str.parse::<DateTime<Utc>>().map_err(|_| {
            ReplayProtectionError::InvalidNonceFormat(format!(
...
pub async fn replay_protection_middleware(
    ...
    if let Err(e) = replay_service
        .validate_request(nonce, timestamp, client_id, path)
        .await
    {
...
fn error_response(error: ReplayProtectionError) -> Response {
    let (status, message) = match error {
        ReplayProtectionError::NonceAlreadyUsed(nonce) => (
            StatusCode::CONFLICT,
            format!("Nonce {} has already been used", nonce),
        ),
        ReplayProtectionError::TimestampTooOld(timestamp) => (
            StatusCode::BAD_REQUEST,
            format!("Request timestamp {} is too old", timestamp),
        ),
        ReplayProtectionError::TimestampTooFuture(timestamp) => (
            StatusCode::BAD_REQUEST,
            format!("Request timestamp {} is too far in the future", timestamp),
        ),
        ReplayProtectionError::InvalidNonceFormat(nonce) => (
            StatusCode::BAD_REQUEST,
            format!("Invalid nonce format: {}", nonce),
        ),
        ReplayProtectionError::MissingNonceOrTimestamp => (
            StatusCode::BAD_REQUEST,
            "Missing required headers: x-nonce and x-timestamp".to_string(),
        ),
...
impl ReplayProtectionMiddlewareFactory {
    ...
    > + Clone {
        ...
        move |request: Request, next: Next| {
            ...
            Box::pin(
                async move { replay_protection_middleware(State(service), request, next).await },
            )
        }
    }
}
...
mod tests {
    ...
    async fn test_replay_protection_validation() {
        ...
        let result = service
            .validate_request(&nonce, timestamp, None, "/api/v1/tips")
            .await;
        assert!(result.is_ok());
    }
}
